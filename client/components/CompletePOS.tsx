                ...
                <div className="bg-gray-50 p-4 rounded-lg shadow-inner overflow-x-auto">
                  <table className="min-w-full border-collapse">
                    ...
                    <tbody>
                      {products.length === 0 ? (
                        <tr>
                          <td colSpan={6} className="border border-gray-200 py-4 text-center text-gray-500">Tidak ada produk. Tambahkan produk baru.</td>
                        </tr>
                      ) : (
                ...
                <div className="bg-gray-50 p-4 rounded-lg shadow-inner overflow-x-auto">
                  <table className="min-w-full border-collapse">
                    ...
                    <tbody>
                      {customers.length === 0 ? (
                        <tr>
                          <td colSpan={4} className="border border-gray-200 py-4 text-center text-gray-500">Tidak ada pelanggan. Tambahkan pelanggan baru.</td>
                        </tr>
                      ) : (
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="bg-white p-4 rounded-md shadow-sm text-gray-700 mb-4">
                        ...
                        <p>Total Pendapatan: <span className="font-bold text-green-600">{formatCurrency(totalRevenue)}</span></p>
                        <p>Jumlah Transaksi: <span className="font-bold">{totalTransactions}</span></p>
                        <p>Rata-rata per Transaksi: <span className="font-bold">{formatCurrency(totalTransactions > 0 ? totalRevenue / totalTransactions : 0)}</span></p>
                      </div>
                    );
                  })()}
                          ...
                          getFilteredSales().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((sale) => (
                            <tr key={sale.id} className="even:bg-gray-50">
                              ...
                              <td className="border border-gray-200 px-3 py-2 text-sm text-gray-500">
                                {sale.paymentMethod === 'cash' ? 'Tunai' : sale.paymentMethod === 'card' ? 'Kartu' : 'Dompet Digital'}
                              </td>
                              <td className="border border-gray-200 px-3 py-2 text-sm">
                  ...
                  {currentShift && currentShift.status === 'open' ? (
                    <div>
                      ...
                      {(() => {
                        ...
                        return <p><strong>Total Pendapatan:</strong> {formatCurrency(salesTotal)}</p>;
                      })()}
                    </div>
                  ) : (
                ...
                <div className="bg-gray-50 p-4 rounded-lg shadow-inner overflow-x-auto">
                  <table className="min-w-full border-collapse">
                    ...
                    <tbody>
                      {users.length === 0 ? (
                        <tr>
                          <td colSpan={3} className="border border-gray-200 py-4 text-center text-gray-500">Tidak ada pengguna. Tambahkan pengguna baru.</td>
                        </tr>
                      ) : (
                        users.map((user) => (
